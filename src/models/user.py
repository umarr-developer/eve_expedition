...
class User(Base):
    ...
    last_result = Column(Integer, default=0)
    ...
    @classmethod
    async def set_result(cls, db_session: sessionmaker, user_id: int, result: int):
        sql = update(cls).where(
            cls.user_id == user_id).values(last_result=result)
        async with db_session() as session:
            await session.execute(sql)
            await session.commit()
        return await cls.get(db_session, user_id=user_id)
